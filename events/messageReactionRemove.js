        ...
        case '🚙':
            reaction.message.guild.members.fetch(user)
                .then(async (member) => {
                    ...
                    console.log('Removed ACC');

                   const tag = await memberDB.findOne({ where: { user_id: member.user.id } });

                   if (tag) {
                       await memberDB.update({ ACC: 0 }, { where: { user_id: member.user.id } });
        ...
        case '🚗':
            reaction.message.guild.members.fetch(user)
                .then(async (member) => {
                    ...
                    console.log('Removed RF2');

                    const tag = await memberDB.findOne({ where: { user_id: member.user.id } });

                   if (tag) {
                       await memberDB.update({ RF2: 0 }, { where: { user_id: member.user.id } });
                       console.log(`User ${member.user.username} has been updated`);
                   }
                })
            break;
        case '🏎️':
            reaction.message.guild.members.fetch(user)
                .then(async (member) => {
                    ...
                    console.log('Removed AMS2');

                    const tag = await memberDB.findOne({ where: { user_id: member.user.id } });

                   if (tag) {
                       await memberDB.update({ AMS2: 0 }, { where: { user_id: member.user.id } });
                       console.log(`User ${member.user.username} has been updated`);
                   }
                })
            break;
